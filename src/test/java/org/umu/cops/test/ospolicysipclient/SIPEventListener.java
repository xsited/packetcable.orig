...
/**
 * This class detects the outsourcing event while acting as as SIP proxy. The
 * event is the reception of a SIP "INVITE" message, meaning that the
 * establishment of a new SIP call is desired. The PEP will use COPS to query
 * the PDP about whether to accept or refuse the call establishment.
 */
public class SIPEventListener extends COPSPepOSEventListener implements
		SipListener {
	...
	/**
	 * Main thread loop. It just registers this object as a SIP listener, and
	 * waits for a keypress. Outsoutcing events will be dispatched in the
	 * meantime by the SIP stack thread.
	 */
	public void run() {
		try {
	...
	/**
	 * Called when the outsourcing event is detected. In this case, the event is
	 * the reception of a SIP request.
	 * 
	 * @param requestEvent
	...
	/**
	 * Called after the decision has been received, to forward the SIP request
	 * that triggered it.
	 * 
	 * @param handle
	 *            The COPS handle for the request
	 */
	public synchronized void forwardPendingRequest(COPSHandle handle) {
		Request sipRequest = (Request) _pendingRequests.remove(handle.getId()
				.toString());
		if (sipRequest != null)
			doSipForward(sipRequest);
	}
	 ...
	 * 
	 * @param sipRequest
	 *            SIP request to be forwarded
	 */
	private void doSipForward(Request sipRequest) {
		try {
			Via myOwnVia = new Via();
			myOwnVia.setBranch(Utils.getInstance().generateBranchId());
	...
	public void processResponse(ResponseEvent responseEvent) {
		...
		System.out.println("Incoming SIP response: "
		...
		try {
			...
			if (viaHeaders.hasNext()) {
				String sentBy = topmostVia.getSentBy().getInetAddress()
						.getHostAddress();
