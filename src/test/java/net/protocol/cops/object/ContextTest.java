 ...
 */
public class ContextTest extends CopsObjectTest {
    ...
    public void testBufferOverflowException() {
        Context e = get(getBuffer(ARRAY));
        e.byteMe(ByteBuffer.allocate(ARRAY.length -1));
    }

    @Override
    protected Context get(ByteBuffer buffer) {
        try {
    ...
    protected Context get() {
        return new Context(Context.RType.CONFIGURATION, 1);
    }
    ...
    private void testLength() {
        ...
        assertEquals(Context.LENGTH, e.getLength());
    }

    private Context get(int mType) {
        return new Context(Context.RType.CONFIGURATION, mType);
    }
}
