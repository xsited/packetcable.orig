...
/**
 * COPSPepReqStateMan manages Request State using Client Handle (RFC 2748 pag.
 * 21) in PEP.
 * 
 * The client handle is used to identify a unique request state for a single PEP
 * per client-type. Client handles are chosen by the PEP and are opaque to the
 * PDP. The PDP simply uses the request handle to uniquely identify the request
 * state for a particular Client-Type over a particular TCP connection and
 * generically tie its decisions to a corresponding request. Client handles are
 * initiated in request messages and are then used by subsequent request,
 * decision, and report messages to reference the same request state. When the
 * PEP is ready to remove a local request state, it will issue a delete message
 * to the PDP for the corresponding client handle. A handle MUST be explicitly
 * deleted by the PEP before it can be used by the PEP to identify a new request
 * state. Handles referring to different request states MUST be unique within
 * the context of a particular TCP connection and client-type.
 * 
...
public class COPSPepReqStateMan {
	...
	/**
	 * The client handle is used to uniquely identify a particular PEP's request
	 * for a client-type
	 */
	...
	protected void initRequestState(Socket sock) throws COPSPepException {
		...
		Hashtable clientSIs=null;
		if (_process != null)
			clientSIs = _process.getClientData(this);
		if (clientSIs == null)
			clientSIs = new Hashtable();
		// Send the request
	...
	protected void processDecision(COPSDecisionMsg dMsg)
			throws COPSPepException {
		// COPSDebug.out(getClass().getName(), "ClientId:" +
		// getClientHandle().getId().str());
		...
		for (Enumeration e = decisions.keys(); e.hasMoreElements();) {
			...
			if (cmddecision.isInstallDecision()) {
				...
				for (; ee.hasMoreElements();) {
					...
					COPSPrObjBase obj = new COPSPrObjBase(decision.getData()
							.getData());
					switch (obj.getSNum()) {
	...
	/**
	 * Process the message SycnStateRequest. The message SycnStateRequest
	 * indicates that the remote PDP wishes the client (which appears in the
	 * common header) to re-send its state.
	 * 
	 * @param ssMsg
	 *            a COPSSyncStateMsg
	 * 
